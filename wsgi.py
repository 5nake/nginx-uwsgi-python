...
def application(environ, start_response):
	...
	#main_page = Template(main_page.render(name=name))
...
def check_auth(environ, start_response):
	...
	try:
		...
		if ident_id_cookie == ident_id_cur:
			SQL = "SELECT user_id from sessions WHERE session_id = %s AND ident_id = %s;"
			data = (session_id_cookie, ident_id_cookie)
			cur.execute(SQL, data)
			current_user_id = cur.fetchall()
			if current_user_id != 0 and current_user_id != None:
				return str(current_user_id[0][0]) #Original current_user_id is smth loke [(1, )] (int)
			else:
